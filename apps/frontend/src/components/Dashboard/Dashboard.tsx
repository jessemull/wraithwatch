                        ...
                        {selectedEntity ? (
                          <div className="space-y-6">
                            ...
                            <div className="border-b border-gray-800 pb-4">
                              <div className="flex items-center space-x-3 mb-3">
                                <div className={`w-4 h-4 rounded-full ${
                                  selectedEntity.type === 'AI_Agent' ? 'bg-blue-500' :
                                  selectedEntity.type === 'Threat' ? 'bg-red-500' :
                                  selectedEntity.type === 'Network_Node' ? 'bg-green-500' :
                                  selectedEntity.type === 'System' ? 'bg-yellow-500' :
                                  selectedEntity.type === 'User' ? 'bg-purple-500' :
                                  'bg-gray-500'
                                }`} />
                            ...
                            {(() => {
                              const renderProperty = (key: string, property: { currentValue: string | number }) => {
                                ...
                                const getStatusColor = (name: string, value: string) => {
                                  if (name === 'routing_status') {
                                    return value === 'optimal' ? 'bg-green-500 text-white' :
                                           value === 'maintenance' ? 'bg-yellow-500 text-black' :
                                           value === 'degraded' ? 'bg-orange-500 text-white' :
                                           'bg-gray-500 text-white';
                                  }
                                  if (name === 'severity') {
                                    return value === 'critical' ? 'bg-red-500 text-white' :
                                           value === 'high' ? 'bg-orange-500 text-white' :
                                           value === 'medium' ? 'bg-yellow-500 text-black' :
                                           value === 'low' ? 'bg-green-500 text-white' :
                                           'bg-gray-500 text-white';
                                  }
                                  if (name === 'mitigation_status') {
                                    return value === 'mitigated' ? 'bg-green-500 text-white' :
                                           value === 'mitigating' ? 'bg-yellow-500 text-black' :
                                           value === 'detected' ? 'bg-red-500 text-white' :
                                           'bg-gray-500 text-white';
                                  }
                                  if (name === 'last_activity') {
                                    return value === 'locked' ? 'bg-red-500 text-white' :
                                           value === 'offline' ? 'bg-gray-500 text-white' :
                                           value === 'active' ? 'bg-green-500 text-white' :
                                           'bg-yellow-500 text-black';
                                  }
                                  if (name === 'permission_level') {
                                    return value === 'super_admin' ? 'bg-red-500 text-white' :
                                           value === 'admin' ? 'bg-orange-500 text-white' :
                                           value === 'user' ? 'bg-blue-500 text-white' :
                                           'bg-gray-500 text-white';
                                  }
                                    ...
                                    {isNumber ? (
                                      <div>
                                        <p className="text-lg font-bold text-white">
                                          {value.toFixed(2)}{getUnit(key)}
                                        </p>
                                        {/* Progress bar for percentage-like values */}
                                        {(key.includes('rate') || key.includes('score') || key.includes('loss') || key.includes('usage')) && (
                                      ...
                                      <div className="flex items-center space-x-2">
                                        {key.includes('status') || key.includes('severity') || key.includes('activity') || key.includes('permission') ? (
                                          <div className={`px-2 py-0.5 rounded text-xs font-medium ${getStatusColor(key, value)}`}>
                                            {value.toUpperCase()}
                                          </div>
                                        ) : (
                              ...
                              const properties = Object.entries(selectedEntity.properties || {})
                                .filter(([key]) => key !== 'source_ip');

                              // Render properties in a 2-column grid
                              return (
                                <div className="grid grid-cols-2 gap-3">
                                  {properties.map(([key, property]) => renderProperty(key, property))}
                                </div>
                              );
                            })()}
